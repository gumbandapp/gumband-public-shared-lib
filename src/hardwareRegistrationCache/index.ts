...
import { ApiVersion, ApplicationInfo, DeviceInfo, PropertyRegistration } from '../types';
 ...
 *
 * Implementations of this class cache the registration messages for future reference and serve as the source to determine whether or not a device
 * is registered.
 *
 * The registration processes managed by this class:
 *  - Device Registration
 *      - This is a device identifying itself to the platform
 ...
 *      - This is required to be completed before device property events can be handled by an mqtt listener
 *
 *  - Application Registration
 *      - This is a device declaring the application metadata and application properties associated with the device
 ...
 */
export interface IHardwareRegistrationCache {
    /**
     * Method to store the API Version of a device in the cache for a given componentId
     *
     * This method is required because the API Version may not always be stored as the
     * 'api_ver' property of the 'device/info' payload.
     *
     ...
     */
    cacheDeviceMQTTAPIVersion(componentId: string, apiVersion: ApiVersion): Promise<void>;

    /**
     * Method to get the API Version of a device in the cache for a given componentId
     *
     * @param {string} componentId - the parsed componentId from the event topic
     ...
     */
    getDeviceMQTTAPIVersion(componentId: string): Promise<ApiVersion | undefined>;

    /**
     * Method to clear the API Version of a device from the cache for a given componentId
     *
     * @param {string} componentId - the parsed componentId from the event topic
     * @returns {Promise<void>}
     * @throws {Error} if the cache fails to remove the data
     */
    clearDeviceMQTTAPIVersion(componentId: string): Promise<void>;

    /**
     * Method to store the DeviceInfo of a device in the cache for a given componentId
     *
     * For the V2 cloud API, this may not be needed. This is here because this is a registration cache
     * and the device info is the V2 payload that communicates the device registration.
     *
     * For future API versions, if the device registration becomes a multi-event process (like the application registration),
     * then this would be useful here.
     *
     * Storing this info in a cache could also be useful to the SDK developers.
     * @param {string} componentId - the parsed componentId from the event topic
     * @param {DeviceInfo} deviceInfo - the parsed device info payload
     * @returns {Promise<void>}
     * @throws {Error} if the cache fails to store the data
     */
    cacheDeviceInfo(componentId: string, deviceInfo: DeviceInfo): Promise<void>;

    /**
     * Method to get the DeviceInfo from the cache for a given componentId
     *
     * @param {string} componentId - the parsed componentId from the event topic
     * @returns {Promise<DeviceInfo | undefined>} - the cached DeviceInfo or undefined if the cache does not contain the data
     * @throws {Error} - if the cache fails to retrieve the data
     */
    getDeviceInfo(componentId: string): Promise<DeviceInfo | undefined>;

    /**
     * Method to remove the DeviceInfo from the cache for a given componentId
     *
     * @param {string} componentId - the parsed componentId from the event topic
     * @returns {Promise<void>}
     * @throws {Error} if the cache fails to remove the data
     */
    clearDeviceInfo(componentId: string): Promise<void>;

    /**
     * Method to store the ApplicationInfo of a device in the cache for a given componentId
     *
    ...
    /**
     * Method to get the ApplicationInfo of a device in the cache for a given componentId
     *
     * @param {string} componentId - the parsed componentId from the event topic
     ...
     * @param {string} componentId - the parsed componentId from the event topic
     * @param {AnySource} source - the source of the message. For example, device or app
     ...
     * This flag acts as a way of saying The application registration is cached and all the device properties are valid.
    ...
    /**
     * Method to remove the info of a device from the cache for a given componentId and source
     *
     * @param {string} componentId - the parsed componentId from the event topic
     * @param {AnySource} source - the source of the message. For example, device or app
     * @returns {Promise<void>}
     ...
     *
     * To be used when the device goes offline.
     * @param {string} componentId - the parsed componentId from the event topic
     * @returns {Promise<void>}
     * @throws {Error} if the cache fails to remove the data
     */
 ...
 * lock and unlock areas of the the device and/or application registration cache for multi-step business
...
export interface ICacheLockByComponentId {
   ...
   lock(componentId: string, timeoutMS?: number): Promise<void>;
     ...
     *
     * This method should release the device registration lock for the componentId if it exists.
     * @param {string} componentId - the parsed componentId from the event topic
