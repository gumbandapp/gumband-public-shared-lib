...
// When the hardware registeres a source (device | app)
 ...
 * MQTT API Version 2
 *
 * This class defines how the listener responds to devices with the api_ver value === 2.
 */
export class MqttApiV2 extends EventEmitter { // eslint-disable-line @typescript-eslint/no-unsafe-declaration-merging
    ...
    constructor (cache: IHardwareRegistrationCache) {
        super();
        this.registrationTimeouts = { 'app': {}, 'device': {} };
        this.cache = cache;
    ...
    async onMessage (componentId: string, topic: V2ApiTopic, payload: Buffer): Promise<void> {
        ...
        switch (topic) {
            case 'device/info':
                await this.handleDeviceInfo(componentId, payload);
                break;
            case 'app/info':
    ...
    /**
     * Called when a V2 device publishes to the 'device/info' topic.
     *
     * @param {string} componentId - parsed componentId
     * @param {V2DeviceInfo | Buffer} payload - published data (JSON parsed or an unprocessed Buffer)
     */
    async handleDeviceInfo (componentId: string, payload: Buffer): Promise<void> {
        // Handle Last Will and Testament
        if (payload.length === 0) {
            this.emit(V2_EVENTS.ONLINE, { componentId, online: false });
            await lockRegistrationCacheAndPerformAction(this.cache, new Set(['device', 'app']), componentId, async () => this.cache.clearAllCacheForComponentId(componentId));
            return;
        }
        ...
        try {
            deviceInfo = await this.packetParser.parseDeviceInfo(payload);
        } catch (reason) {
            let message = `Failed to unpack device/info payload for componentId: ${componentId}`;
            ...
            console.debug('Clearing Device and Application Registration from cache for componentId:', componentId);
            try {
                await lockRegistrationCacheAndPerformAction(this.cache, new Set(['device', 'app']), componentId, async () => this.cache.clearAllCacheForComponentId(componentId));
            } catch (e) {
                const message = `Failed to clear device info cache for componentId: ${componentId}`;
        ...
        try {
            await lockRegistrationCacheAndPerformAction(this.cache, new Set(['device']), componentId, async () => {
                await this.cache.cacheDeviceMQTTAPIVersion(componentId, deviceInfo.api_ver);
                await this.cache.cacheDeviceInfo(componentId, deviceInfo);
            });
        } catch (e) {
            const message = `Failed to cache device info registration for componentId: ${componentId}`;
            console.error(message);
            if (isNativeError(e)) {
                console.error(e.message);
            } else {
            ...
            //   - Re-request device info from hardware?
        }

        this.setRegistrationTimeoutForComponentIdAndSource(componentId, 'device');
    }
     ...
     *
     * @param {string} componentId - parsed componentId
     * @param {V2Source} source - the source of the message. For example, device or app
     * @param {Buffer} payload - published payload Buffer
     * @return {void}
     */
    ...
    async handlePropertyValueUpdate (componentId: string, source: V2Source, propertyInfo: Array<string>, payload: Buffer): Promise<void> {
        ...
        try {
            ...
            console.log(`format: "${registeredProperty.format}", unpackedPayload: ${JSON.stringify(unpackedPayload)}`);
    ...
    private setRegistrationTimeoutForComponentIdAndSource (componentId: string, source: AnySource): void {
        ...
        }, 30 * 1000); // TODO: Is 30 seconds enough time to receive all the packets from the device?
    }
    ...
    async checkRegistrationForComponentIdAndSource (componentId: string, source: AnySource): Promise<void> {
        await lockRegistrationCacheAndPerformAction(this.cache, new Set([source]), componentId, async () => {
            ...
            let sourceInfo: ApplicationInfo | DeviceInfo | undefined;
            try {
                switch (source) {
                    case 'device':
                        sourceInfo = await this.cache.getDeviceInfo(componentId);
                        break;
                    case 'app':
 ...
 * Registration event.
 *
 * This is event is emit when a source's registration state changes for a device.
 *
