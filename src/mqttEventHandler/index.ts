...
import type {
    ApiVersion,
    MQTTInitialRegistrationTopic,
} from '../types';
import { V2ApiTopic, isMQTTInitialRegistrationTopic } from '../types';
...
export class MQTTEventHandler extends EventEmitter {
    private cache: IHardwareRegistrationCache;
    public mqttV2Api: MqttApiV2;
    ...
    async onMessage (componentId: string, topic: string, payload: Buffer): Promise<void> {
        ...
        try {
            apiVersion = await this.cache.getDeviceMQTTAPIVersion(componentId);
        } catch (e) {
            const message = `Unable to get Device MQTT API Version from cache for componentId: ${componentId}`;
    ...
    /**
     * Called when a device registers itself with gumband.
     *
    ...
    private async getApiVersionFromRegistrationTopic (_componentId: string, topic: MQTTInitialRegistrationTopic, payload: Buffer): Promise<ApiVersion | undefined> {
        console.debug('baseMqttPacketHandler.registerDevice()');
        switch (topic) {
            case 'device/info':
            {
                let jsonPayload;
